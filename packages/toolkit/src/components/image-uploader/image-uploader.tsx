 ...
 * ```
 */
const ImageUploader = forwardRef<HTMLInputElement, ImageUploaderProps>(({ imageProcessLoader = <>
      loading...
    </>, maxHeight, processedImageMetadata, ...otherProps }, ref): JSX.Element => {
  ...
  const handleImageMetadata = (url: string): void => {
    setImageMetadata(url, maxHeight).then((imageMetadata) => {
