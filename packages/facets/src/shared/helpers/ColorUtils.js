...
export const alterLuminosity = memoizee(
  _alterLuminosity,
  function (colorValue, newLuminosity) {
...
export const fullColorNumber = memoizee(
  function fullColorNumber(brandKey: string | void, colorNumber: string | void, brandKeyNumberSeparator = ' '): string {
    return [brandKey, colorNumber].filter(Boolean).join(brandKeyNumberSeparator)
  },
  { primitive: true, length: 2 }
)

export const fullColorName = memoizee(
  function fullColorName(
    brandKey: string | void,
    colorNumber: string | void,
    name: string,
    brandKeyNumberSeparator: string | void
  ): string {
...
export const generateColorDetailsPageUrl = memoizee(function generateColorDetailsPageUrl(color: Color): string {
  ...
  return (
    '/' + compact([ROUTE_PARAMS.ACTIVE, ROUTE_PARAMS.COLOR_DETAIL, color.id, `${colorNumber}-${colorName}`]).join('/')
  )
})

export const generateColorWallPageUrl = memoizee(
  (sectionName = '', familyName = '', colorId = '', colorSEO = ''): string => {
    return `/${compact([
      ROUTE_PARAMS.ACTIVE,
      ...
      colorId && `${ROUTE_PARAMS.COLOR}/${colorId}`,
      kebabCase(colorSEO)
    ]).join('/')}/`
  },
  { primitive: true, length: 3 }
...
export const getContrastYIQ = memoizee(
  (hexcolor: string) => {
...
export const cleanColorNameForURL = (name: string = ''): string =>
  kebabCase(
    name
      .split('-')
      .map((v = '') => {
