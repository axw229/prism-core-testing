...
export const getCoords = memoizee((arr: any[], val): number[] => {
  if (!Array.isArray(arr)) {
    ...
    return (
      arr
        .map((subArr: any[], i: number): number[] => [i, ...getCoords(subArr, val)])
        .find((coords) => coords[1] !== -1) || [-1]
    )
  } else {
    return [arr.indexOf(val)]
  }
})
export const getLongestArrayIn2dArray = memoizee((arr: any[][]): number =>
  arr ? arr.reduce((n, arr) => Math.max(n, arr.length), 0) : 0
)

// return the total width (in cells) of a chunk row
const getWidthOfChunkRow = (chunkRow: string[][][]): number =>
  chunkRow.reduce((width, chunk) => width + getLongestArrayIn2dArray(chunk), 0)
...
export const getWidthOfWidestChunkRowInChunkGrid = memoizee((grid: string[][][][] | null | undefined): number => {
  return grid ? grid.reduce((widest, chunkRow) => Math.max(widest, getWidthOfChunkRow(chunkRow)), 0) : 0
})
export const getLevelMap = memoizee(
  (chunkGrid: string[][][][], bloomEnabled: boolean, centerId: string | null | undefined): Record<string, number> => {
    if (!centerId) return {}

    // find chunk with active centerId
    for (const chunk of flatten(chunkGrid)) {
      const [row, column] = getCoords(chunk, centerId)
      ...
      const setLevels = (level: number, indexes: Array<[number, number]>): void => {
        indexes.forEach(
          ([row, column]: [number, number]) =>
            chunk[row]?.[column] !== undefined && (levelMap[chunk[row][column]] = level)
        )
      }
...
export const rowHasLabels = memoizee(
  (grid: string[][][][], rowIndex: number, labels: Array<string | null | undefined> | null | undefined): boolean => {
    if (grid === undefined || labels === undefined || grid.length <= rowIndex) {
      return false
    }

    const numElementsBeforeRow: number = flatten(take(grid, rowIndex)).length
    return (
      labels.length > numElementsBeforeRow &&
      take(drop(labels, numElementsBeforeRow), grid[rowIndex].length).some((label) => label !== undefined)
    )
  }
)
 ...
 */
export const getTotalGridHeight = (
  grid: string[][][][],
  paddingBetweenChunks: number,
  labels: Array<string | null | undefined>
): number =>
  grid.reduce(
    (h, chunkRow, i) =>
      h + paddingBetweenChunks + getHeightOfChunkRow(chunkRow) + (rowHasLabels(grid, i, labels) ? 1.6 : 0),
    0
  ) + 2
 ...
 */
export const getTotalGridWidth = (grid: string[][][][], paddingBetweenChunks: number): number =>
  grid.reduce((w, chunkRow) => Math.max(w, getWidthOfChunkRow(chunkRow) + paddingBetweenChunks * chunkRow.length), 0) +
  2
...
export const computeFinalScrollPosition = (
  ...
  const heightOfChunksAbove =
    take(grid, chunkRow).reduce(
      (h, chunkRow, i) =>
        h + paddingBetweenChunks + getHeightOfChunkRow(chunkRow) + (rowHasLabels(grid, i, labels) ? 2 : 0),
      0
    ) + 2
...
export const makeChunkGrid = memoizee(
  (
    ...
    const wrapChunks = (chunkedChunks: string[][][]): string[][][][] => {
      return wrappingEnabled // determine gridWidth by decreasing it until it is less than the containerWidth
        ? chunk(
            chunkedChunks,
            rangeRight(1, gridWidth + 1).find(
...
export const calculateLabelHeight = (cellSize: number): number => {
  if (cellSize > 29.2 || (cellSize <= 24 && cellSize > 20.9)) {
    return cellSize
